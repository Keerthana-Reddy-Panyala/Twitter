...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  const getTweetsListQuery = `SELECT tweet.tweet,COUNT(like.like_id) AS likes,COUNT(reply.reply_id) AS replies,tweet.date_time AS dateTime
  FROM tweet
  LEFT JOIN like ON tweet.tweet_id=like.tweet_id
  LEFT JOIN reply ON tweet.tweet_id=reply.tweet_id
  WHERE tweet.tweet_id IN (${getTweetArray})
  GROUP BY tweet.tweet_id;`;
  const tweets = await database.all(getTweetsListQuery);
  const tweetsArray = tweets.map((eachTweet) => {
    return {
